if SERVER and SERVER_IP ~= "46.174.53.204" and SERVER_IP ~= "37.230.162.62" then return end
...
local patt = "Ошибка загрузки автодоната #%d\nError: %s\n"
local function downloadSuperfile(version, cb, _failure)
	...
	if _failure == 1 then
		-- ErrorNoHalt("[IGS] #" .. _failure .. " повторение загрузки", url)
		url = "https://unionrp.info/igs/igs_proxy?repo=" .. IGS_REPO .. "?version=" .. version
	elseif _failure == 2 then
		url = "https://unionrp.info/igs/superfile.json"
	elseif _failure == 3 then
		url = "https://gh.gm-donate.net/" .. IGS_REPO .. "/releases/download/" .. version .. "/superfile.json"
	end

	fetchDelayed((_failure or 0) * 5, url, function(superfile)
		...
		cb(superfile)
	end, function(err) -- timeout, unsuccessful
		if not _failure or _failure <= 3 then
			downloadSuperfile(version, cb, (_failure or 0) + 1)
			return
		end

		error(patt:format(_failure, err))
	end)
end
