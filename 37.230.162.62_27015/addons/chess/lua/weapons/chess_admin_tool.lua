...
function SWEP:PrimaryAttack()
	if SERVER and IsValid( self:GetOwner() ) then
		if not self:GetOwner():IsAdmin() then
			self:GetOwner():ChatPrint( "You are not allowed to use this tool!" )
			self:Remove()
			return
		end
...
if CLIENT then
	...
	function SWEP:PostDrawViewModel()
		if LocalPlayer()~=self:GetOwner() then return self.BaseClass.PostDrawViewModel( self ) end
		self:DoGhosts()
	end
	function SWEP:DrawWorldModel()
		if LocalPlayer()~=self:GetOwner() then return self.BaseClass.DrawWorldModel( self ) end
		self:DrawModel()
		self:DoGhosts()
	end
end