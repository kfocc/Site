...
TOOL.DetailWhitelist = {"concrete", "metal", "plaster", "rock"}
...
function TOOL:LeftClick(trace)
	...
	local texture = string.lower(string.Trim(self:GetClientInfo("texture")))
	if materials.blackList[texture] then return false end
...
function TOOL:RightClick(trace)
	...
	if IsValid(trEnt) and trEnt:GetMaterial() ~= "" and trEnt:GetMaterial():sub(1, 1) ~= "!" then
		bIsMat = true
	end

	if not bIsMat and trace.HitTexture[1] == "*" and not trEnt.MaterialData then return false end
	...
	local data = trEnt.MaterialData or {
		texture = bIsMat and trEnt:GetMaterial() or trace.HitTexture,
		scalex = 1,
		scaley = 1,
		offsetx = 0,
		offsety = 0,
...
if CLIENT then
	...
	list.Set("tool.advmat.details", "#tool.advmat.details.concrete", {
		advmat_noisetexture = "concrete"
	})

	list.Set("tool.advmat.details", "#tool.advmat.details.metal", {
		advmat_noisetexture = "metal"
	})

	list.Set("tool.advmat.details", "#tool.advmat.details.plaster", {
		advmat_noisetexture = "plaster"
	})

	list.Set("tool.advmat.details", "#tool.advmat.details.rock", {
		advmat_noisetexture = "rock"
	})
end