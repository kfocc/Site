...
local oldMaxCount = CreateConVar("stacker_max_count", 1, cvarFlagsNotif, "Defines the max amount of props that can be stacked at a time")
...
function TOOL:ApplyMaterial(ent, material)
	...
	if not list.Contains("OverrideMaterials", material) and material ~= "" then return end
...
if CLIENT then
	...
	--]]
	--
	hook.Add("PostDrawEffects", mode .. "_predrawhalos", function()
	...
	hook.Add("PostDrawTranslucentRenderables", mode .. "_directions", function(drawingDepth, drawingSky)
		if drawingDepth or drawingSky then return end
		-- check if the player has fully initialized
		local ply = LocalPlayer()
		if not IsValid(ply) then return end
...
if CLIENT then
	...
	local function buildCPanel(cpanel)
		...
		local presets = {
			...
			CVars = {mode .. "_mode", mode .. "_direction", mode .. "_count", --[[mode .. "_freeze", mode .. "_weld", mode .. "_nocollide", mode .. "_ghostall",--]] mode .. "_material", mode .. "_physprop", mode .. "_color", mode .. "_offsetx", mode .. "_offsety", mode .. "_offsetz", mode .. "_pitch", mode .. "_yaw", mode .. "_roll", mode .. "_relative", mode .. "_draw_halos", mode .. "_halo_r", mode .. "_halo_g", mode .. "_halo_b", mode .. "_halo_a", mode .. "_draw_axis", mode .. "_axis_labels", mode .. "_axis_angles",}
		}

		local relativeOptions = {
			[L(prefix .. "combobox_world")] = {
				[mode .. "_mode"] = improvedstacker.MODE_WORLD
			},
			[L(prefix .. "combobox_prop")] = {
				[mode .. "_mode"] = improvedstacker.MODE_PROP
			},
		}
		...
		local directionOptions = {
			["1 - " .. L(prefix .. "combobox_direction_front")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_FRONT
			},
			["2 - " .. L(prefix .. "combobox_direction_back")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_BACK
			},
			["3 - " .. L(prefix .. "combobox_direction_right")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_RIGHT
			},
			["4 - " .. L(prefix .. "combobox_direction_left")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_LEFT
			},
			["5 - " .. L(prefix .. "combobox_direction_up")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_UP
			},
			["6 - " .. L(prefix .. "combobox_direction_down")] = {
				[mode .. "_direction"] = improvedstacker.DIRECTION_DOWN
			},
		}
		...
		cpanel:AddControl("Slider", {
			...
			Value = 0,
			Command = mode .. "_offsetz"
		})

		cpanel:AddControl("Button", {
			Label = L(prefix .. "label_reset_angles"),
			Command = mode .. "_reset_angles"
		})
		...
		cpanel:AddControl("Button", {
			...
			Command = mode .. "_show_settings"
		})

		if showSettings then
			cpanel:AddControl("Checkbox", {
				Label = L(prefix .. "checkbox_use_shift_key"),
				Command = mode .. "_use_shift_key",
	...
	hook.Add("PopulateToolMenu", mode .. "AdminUtilities", function()
		spawnmenu.AddToolMenuOption("Utilities", "Admin", mode .. "_utils", L(prefix .. "name"), "", "", function(cpanel)
			...
			local presets = {
				...
				cvars = {
					{
						CVar = mode .. "_max_per_player",
						CCmd = mode .. "_set_max_per_player"
					},
					{
						CVar = mode .. "_max_per_stack",
						CCmd = mode .. "_set_max_per_stack"
					},
					{
						CVar = mode .. "_delay",
						CCmd = mode .. "_set_delay"
					},
					{
						CVar = mode .. "_max_offsetx",
						CCmd = mode .. "_set_max_offsetx"
					},
					{
						CVar = mode .. "_max_offsety",
						CCmd = mode .. "_set_max_offsety"
					},
					{
						CVar = mode .. "_max_offsetz",
						CCmd = mode .. "_set_max_offsetz"
					},
					-- {
					-- 	CVar = mode .. "_force_freeze",
					-- 	CCmd = mode .. "_set_force_freeze"
					-- },
					-- {
					-- 	CVar = mode .. "_force_weld",
					-- 	CCmd = mode .. "_set_force_weld"
					-- },
					-- {
					-- 	CVar = mode .. "_force_nocollide",
					-- 	CCmd = mode .. "_set_force_nocollide"
					-- },
					{
						CVar = mode .. "_force_stayinworld",
						CCmd = mode .. "_set_force_stayinworld"
					},
				},
			}
			...
			for k, data in pairs(checkboxes) do
				...
				function cb:OnChange(bool)
					RunConsoleCommand(mode .. "_set_force_" .. data, bool and "1" or "0")
				end
