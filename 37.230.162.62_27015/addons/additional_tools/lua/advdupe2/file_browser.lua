...
local function tableSortNodes(tbl)
	for k, v in ipairs(tbl) do
		tbl[k] = {string.lower(v.Label:GetText()), v}
	end
	...
	if node.Derma.ClassName == "advdupe2_browser_file" then
		...
		Menu:AddOption("Save", function()
			...
			parent.Submit.DoClick = function()
				...
				AdvDupe2.Description = desc
				AddHistory(name)

				net.Start("AdvDupe2_SaveFile")
...
local function UpdateClientFiles()
	...
	PurgeFiles(AdvDupe2.DataFolder .. "/", pnlCanvas:AddFolder("-Your Files-"))
...
function PANEL:Init()
	...
	self.FileName.OnTextChanged = function()
		if self.FileName.FirstChar then
			if string.lower(self.FileName:GetValue()[1] or "") == string.lower(input.LookupBinding("menu")) then
