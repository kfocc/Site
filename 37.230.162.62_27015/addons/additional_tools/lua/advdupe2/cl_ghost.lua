...
hook.Add("PreDrawTranslucentRenderables", "Ghosts.DrawLine", function(bDepth, bSkybox)
	if bDepth or bSkybox then return end

	local headGhost = AdvDupe2.HeadGhost
	if not IsValid(headGhost) then return end

	local entPos = headGhost:GetPos()
	local eyePos = LocalPlayer():GetPos()
	render.DrawLine(eyePos, entPos, color_red, false)
end)

hook.Add("HUDPaint", "Ghosts.DrawPosDist", function()
	local headGhost = AdvDupe2.HeadGhost
	if not IsValid(headGhost) then return end

	local entPos = headGhost:GetPos()
	local struct = entPos:ToScreen()
	if not struct.visible then return end

	local lpPos = LocalPlayer():GetPos()
	local sqrDist = entPos:DistToSqr(lpPos)
	local dist = ConvertDist(sqrDist) .. "м"

	local color = color_white
	if sqrDist >= AdvDupe2.distToPaste then
		color = color_red
	end
	draw.SimpleText(dist, "DermaLarge", struct.x, struct.y, color)
end)