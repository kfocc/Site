...
slap:addParam{ type=ULib.cmds.PlayerArg }
...
whip:addParam{ type=ULib.cmds.PlayerArg }
...
ignite:addParam{ type=ULib.cmds.PlayerArg }
...
------------------------------ Freeze ------------------------------
function ulx.freeze( calling_ply, target_plys, should_unfreeze, hide_notify)
	local affected_plys = {}

		if not should_unfreeze and ulx.getExclusive( target_plys, calling_ply ) then
			ULib.tsayError( calling_ply, ulx.getExclusive( target_plys, calling_ply ), true )
		else
			local v = target_plys
			if v:InVehicle() then
				v:ExitVehicle()
...
freeze:addParam{ type=ULib.cmds.PlayerArg }
...
function ulx.god( calling_ply, target_plys, should_revoke )
	if not IsValid(target_plys) then
...
god:addParam{ type=ULib.cmds.PlayerArg, ULib.cmds.optional }
...
------------------------------ Cloak ------------------------------
function ulx.cloak( calling_ply, target_plys, should_uncloak )
	if not target_plys:IsValid() then
		Msg( "You are always invisible.\n" )
		return
	end
	if calling_ply != target_plys and not should_uncloak and not calling_ply:IsSuperAdmin() then AAN(string.format("%s (%s) попытался выдать клок %s (%s)", calling_ply:Name(), calling_ply:SteamID(), target_plys:Name(), target_plys:SteamID())) calling_ply:Kick("AdminAbuse. Next - Warn") return end


		ULib.invisible( target_plys, not should_uncloak )

	if not should_uncloak then
		ulx.fancyLogAdmin( calling_ply, "#A cloaked #T", target_plys )
	else
		ulx.fancyLogAdmin( calling_ply, "#A uncloaked #T", target_plys )
	end
end
local cloak = ulx.command( CATEGORY_NAME, "ulx cloak", ulx.cloak, "!cloak" )
cloak:addParam{ type=ULib.cmds.PlayerArg, ULib.cmds.optional }
...
blind:addParam{ type=ULib.cmds.PlayerArg }
...
function ulx.jail( calling_ply, target_plys, seconds, should_unjail )
	local affected_plys = {}
		...
		if not should_unjail then
			...
			elseif not jailableArea( v:GetPos() ) then
				ULib.tsayError( calling_ply,"Член к ", v:Nick() .. " поставить не выйдет!", true )
			else
				doJail( v, seconds )
	...
	if not should_unjail then
		local str = "#A отправил #T в членокомнату"
		if seconds > 0 then
			str = str .. " на #i сек"
		end
		ulx.fancyLogAdmin( calling_ply, str, affected_plys, seconds )
	else
		ulx.fancyLogAdmin( calling_ply, "#A освободил #T из членокомнаты", affected_plys )
	end
end
local jail = ulx.command( CATEGORY_NAME, "ulx penisjail", ulx.jail, "!penisjail" )
...
jail:setOpposite( "ulx unpenisjail", {_, _, _, true}, "!unpenisjail" )
...
function ulx.jailtp( calling_ply, target_ply, seconds )
	...
	elseif not jailableArea( pos ) then
		ULib.tsayError( calling_ply, "Нету места для члена!", true )
		return
	else
	...
	local str = "#A телепортировал #T в членокомнату"
	if seconds > 0 then
		str = str .. " на #i секунд"
	end
	ulx.fancyLogAdmin( calling_ply, str, target_ply, seconds )
end
local jailtp = ulx.command( CATEGORY_NAME, "ulx penisjailtp", ulx.jailtp, "!penisjailtp" )
...
local mdl1 = Model( "models/props_interiors/vendingmachinesoda01a_door.mdl" )
local jail = {
	{ pos = Vector( 0, 0, -5 ), ang = Angle( 90, 0, 0 ), mdl=mdl1 },
	{ pos = Vector( 0, 0, 97 ), ang = Angle( 90, 0, 0 ), mdl="models/props_c17/gravestone001a.mdl" },
	{ pos = Vector( -21, 30, 97 ), ang = Angle( 90, 0, 0 ), mdl="models/props/de_nuke/clock.mdl" },
	{ pos = Vector( -21, -30, 97 ), ang = Angle( 90, 0, 0 ), mdl="models/props/de_nuke/clock.mdl" },
...
ragdoll:addParam{ type=ULib.cmds.PlayerArg }
...
maul:addParam{ type=ULib.cmds.PlayerArg }
...
strip:addParam{ type=ULib.cmds.PlayerArg }
strip:defaultAccess( ULib.ACCESS_ADMIN )
strip:help( "Strip weapons from target(s)." )


if SERVER then
	local function RemoveFreeze(ply)
		if ply.frozen then
			ply:UnLock()
			ply.frozen = nil
			ulx.clearExclusive(ply)
			ply:DisallowSpawning(false)
			ulx.setNoDie( ply, false )
		end
	end
	hook.Add("PlayerSpawn", "ulx.RemoveFreeze", RemoveFreeze)
	hook.Add("PostPlayerDeath", "ulx.RemoveFreeze", RemoveFreeze)
end