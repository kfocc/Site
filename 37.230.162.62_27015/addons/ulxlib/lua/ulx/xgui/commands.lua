...
function cmds.refreshPlist( arg )
	...
	for _, ply in ipairs( targets ) do
		local name, group = ply:Nick(), ply:GetUserGroup()
		--if IsValid(ply:GetNetVar("FakePlayer")) then ply = ply:GetNetVar("FakePlayer") end
		if ply:GetNetVar("FakeGroup") != "" then group = ply:GetNetVar("FakeGroup") end
		if ply:GetNetVar("MaskMe") then continue end
		local line = cmds.plist:AddLine( name, group )
		line.ply = ply
