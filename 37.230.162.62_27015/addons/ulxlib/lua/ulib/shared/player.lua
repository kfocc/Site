		...
		if piece ~= "" then
			...
			if enable_keywords then
				...
				elseif piece:sub( 1, 1 ) == "@" then
					if #piece == 1 then
						if IsValid( ply ) then
						local player = ply:GetEyeTrace().Entity
...
function ULib.getUser( target, enable_keywords, ply )
	...
	-- First, do a full name match in case someone's trying to exploit our target system
	for _, player in ipairs( players ) do
		if target == player:Nick():lower() or target == player:SteamName():lower() or target == player:SteamID() or target == player:GetID() then
	...
	for _, player in ipairs( players ) do
		if player:Nick():lower():find( target, 1, true ) or player:SteamName():lower():find( target, 1, true ) or player:SteamID():find( target, 1, true ) or player:GetID():find( target, 1, true )  then -- No patterns
