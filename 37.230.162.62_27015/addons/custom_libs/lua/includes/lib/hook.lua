...
--[[---------------------------------------------------------
	Name: Exists
	Args: string identifier, hookName
	Desc: Find the hook from list.
-----------------------------------------------------------]]

local function Exists(id, event_name)
	local event_list = events[id]
	if not event_list then return end

	return find_hook(event_list, event_name) and true or false
end
...
-----------------------------------------------------------]]
local function Add(event_name, name, func, priority)
...
-----------------------------------------------------------]]
local function Call(event_name, gm, ...)
	local event = events[event_name]
	if event then
		local i, n = 2, event.n
		::loop::
