...
SWEP.PrintName				= "Арбалет"		-- Weapon name (Shown on HUD)
...
SWEP.Primary.KickUp			= 0.245				-- Maximum up recoil (rise)
SWEP.Primary.KickDown		= 0.245		-- Maximum down recoil (skeet)
SWEP.Primary.KickHorizontal	= 0.245		-- Maximum up recoil (stock)
...
SWEP.EventTable = {
  [ACT_VM_PRIMARYATTACK] = {
    { time = 0.25, type = "lua", value = function( wep ) if wep:Ammo1() > 0 then wep:Reload() end end, client = true, server = true },
    { time = 0, type = "lua", value = function( wep ) wep:GetOwner():EmitSound("weapons/crossbow/bolt_fly4.wav") end, client = false, server = true }
  },
  [ACT_VM_RELOAD] = {
    {time = 0.55, type = "sound", value = Sound("Crossbow.Reload")},
  },
}
...
function SWEP:Reload(a)
  if self:Clip1() == 1 then return end
  BaseClass.Reload(self, a)
end
...
SWEP.Primary.SpreadMultiplierMax = 1.045

SWEP.IronRecoilMultiplier = 0.545
SWEP.CrouchAccuracyMultiplier = 0.545
