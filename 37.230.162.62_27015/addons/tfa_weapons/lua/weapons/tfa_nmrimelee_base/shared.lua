...
SWEP.Primary.Ammo = ""
SWEP.Primary.ClipSize = CurTime()+ 2
...
function SWEP:Deploy()
	...
	self:GetOwner().LastNMRIMSwing = nil
	self:GetOwner().HasTFANMRIMSwing = false
	return true
end
...
function SWEP:Think2()
	...
	if self.Primary.Motorized then
		...
		if OnStatus[stat] then
			if SERVER then
				self.AmmoDrainDelta = self.AmmoDrainDelta + ( self:GetOwner():KeyDown(IN_ATTACK) and self.Primary.Motorized_AmmoConsumption_Saw or self.Primary.Motorized_AmmoConsumption_Idle ) * TFA.FrameTime()
		...
		if stat == TFA.GetStatus("NMRIH_MELEE_MOTOR_START") and CurTime() > self:GetStatusEnd() then
			...
			self:PlayIdleSound( false )
			self:CompleteReload()
		elseif stat == TFA.GetStatus("NMRIH_MELEE_MOTOR_LOOP") and self:GetOwner():KeyDown(IN_ATTACK) then
		...
		elseif stat == TFA.GetStatus("NMRIH_MELEE_MOTOR_ATTACK") and not self:GetOwner():KeyDown(IN_ATTACK) then
			stat = TFA.GetStatus("NMRIH_MELEE_MOTOR_LOOP")
			self:SetStatus( stat )
			self:SetStatusEnd( math.huge )
	...
	if stat == TFA.GetStatus("NMRIH_MELEE_CHARGE_LOOP") and not self:GetOwner():KeyDown(IN_ATTACK) then
...
function SWEP:HandleDelayedAttack( statv )
	if statv == 0 then return end
	--print("statv", statv)
	self.NextSwingSoundTime = self.NextSwingSoundTime or 1
	self.NextSwingSoundTime_Hard = self.NextSwingSoundTime_Hard or 1
	self.NextSwingTime_Hard = self.NextSwingTime_Hard or 1
	self.NextSwingTime = self.NextSwingTime or 1
	--print(CurTime(),self.NextSwingTime, self.NextSwingTime_Hard, self.NextSwingSoundTime)
	stat = statv
	--print("check", TFA.GetStatus("NMRIH_MELEE_SWING"), "has", stat)
	...
	if stat == TFA.GetStatus("NMRIH_MELEE_CHARGE_END") then
		...
		if self.NextSwingTime_Hard > 0 and CurTime() > self.NextSwingTime_Hard then
			...
			self.NextSwingTime_Hard = CurTime() + 2
		end
		if self.NextSwingSoundTime_Hard > 0 and CurTime() > self.NextSwingSoundTime_Hard then
			...
			self.NextSwingSoundTime_Hard = CurTime() + 2
		end
...
function SWEP:PrimaryAttack()
	...
	self:SetNextPrimaryFire(CurTime() + 2) -- attack time hardcoded for ALL weapons? lol
...
function SWEP:SecondaryAttack()
	if self:GetStatus() ~= TFA.Enum.STATUS_IDLE then return end
	if self:IsSafety() then return end
	self:SetNextSecondaryFire(CurTime() + 2) -- attack time hardcoded for ALL weapons? lol
...
function SWEP:HitThing( damage, force, reach, blunt, sndtype )
	...
	-- if slashtrace.Hit then
		...
		-- 	dmg:SetDamageForce(self:GetOwner():GetAimVector() * (damage * 0.25))
...
function SWEP:PlayIdleSound( sawing )
	...
	if not self.HasPlayedSound[ sndid ] then
		...
		self:EmitSound( sawing and self.Primary.Motorized_SawSound or self.Primary.Motorized_IdleSound, self.Primary.Motorized_Volume or .85)
	end
end
...
function SWEP:SwingThirdPerson()
	if self.HoldType == "melee" or self.HoldType == "melee2" then
		...
		self:GetOwner():AnimRestartGesture(0, ACT_HL2MP_GESTURE_RANGE_ATTACK_MELEE2, true)
	end
end
