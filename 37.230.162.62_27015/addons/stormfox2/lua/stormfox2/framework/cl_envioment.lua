...
---------------------------------------------------------------------------]]
	-- Local functions
		...
		local function IsRoofAngle( Ang )
			return Ang:Forward():Dot(vector_up) > 0.3
		end
			...
			if surfaceinfos[WATER_VERTS] then
				...
				for i,v in ipairs(surfaceinfos[WATER_VERTS]) do
					...
					elseif CLIENT then
						...
						for i = 1,3 do
							local vec = t[i]
							table.insert(STORMFOX_WATERMESHBUILD_SKYBOX, {pos = vec + (vec_ex / s),u = vec.x / ice_size * s,v = vec.y / ice_size * s, normal = vector_up})
						end
						for i = 4,20 do
							if #t < i then continue end
							local vec = t[1]
...
local env_update_stage = 0

local function update_environment()
	local view = StormFox2.util.GetCalcView()
	viewPos = view.pos

	-- Stage 0: Update water and window checks
	if env_update_stage == 0 then
		...
		if bit.band(util.PointContents(viewPos), CONTENTS_WATER) == CONTENTS_WATER then
			...
			in_water = t.FractionLeftSolid and (viewPos.z + 1000 * t.FractionLeftSolid) or (viewPos.z + 1000)
		else
			in_water = nil
		end
		...
		for _, v in ipairs(glass_dynamic) do -- ent,c
			if v:IsAlive() and viewPos:Distance(v:GetCenter()) <= 10000 then
		...
		z_distance = tr.Hit and (tr.Fraction * 16000) or 16000

		-- Locate nearest outside / window
		if not is_inside or in_water then
			-- Skip window and outside checks if we're not inside or in water
			return
		end
	...
	-- Stage 1: Update nearest window
	if env_update_stage == 1 then
		if surfaceinfos[GLASS_VERTS] then
			...
			for i = 1, 10 do
				...
				local v, tr = MatchTrace(viewPos, winpos, surfinfo and surfinfo:GetNormal())
				if v then
			...
			-- Check ents
			local curDist = nearest_window and nearest_window:DistToSqr(viewPos)
			for i = 1, #close_window_ents do
				local ent = close_window_ents[i]
				if not ent:IsAlive() then continue end

				local center = ent:GetCenter()
				local dis = center:DistToSqr(viewPos)
				if curDist and dis > curDist then continue end

				local n = view.ang:Forward()
				local v, tr = MatchTrace(viewPos, center - n * 15)
				if v then
					curDist = dis
					nearest_window = center
				end
			end
		end
	...
	-- Stage 2: Update nearest outside point
	if env_update_stage == 2 then
		...
		if not nearest_outside then
			...
			local r = 360 / lines
			for i = 1, lines do
				nearest_outside = SkyLine(viewPos, Angle(0, r * i, 0):Forward(), 5, 600, nil, MASK_SHOT)
				if nearest_outside then break end
			end
		end

		-- If the oldpos is still valid. Check to see if the currentpos is closer.
		if oldpos and EasyTrace(viewPos, oldpos)
		and (not nearest_outside or oldpos:DistToSqr(viewPos) < nearest_outside:DistToSqr(viewPos)) then -- If old pos and we still see old pos
