...
-- Load materials
	...
	-- local function safeCall(w, h, layer)
	--	 StormFox2.Skybox2D.CloudLayerRender(w, h, layer)
	-- end
	local function UpdateCloudMaterial(layer,cloud_alpha)
		...
		-- Render RT texture
			...
			surface.SetDrawColor(255,255,255,cloud_alpha)
		...
		-- Mask RT tex
		--	RTMask()
		--		surface.SetDrawColor(255,255,255,255 - cloud_alpha)
		--		surface.SetMaterial(cloudbig)
		--		DrawTextureRectWindow(texscale,texscale,offset[layer][1] + d_seed,offset[layer][2] + d_seed)
...
function StormFox2.Skybox2D.CloudLayer(eye)
	...
	--if cl_amd <= 0 then return end
		...
		for i = 1,n do
			...
			RenderDome(vector_up * (z_level + 0.4 * ri) + eye * eye_mult,sky_mats[i],255)
		end
end