...
do
	...
	rain:RenderWindowRefract64x64(function(w, h)
		...
		-- Base
		surface.SetDrawColor(255,255,255,255 * P)
...
-- Render Filter (Screen filter, this is additive)
	...
	local rx,ry,rx2,ry2 = 0,0,0,0
		--surface.SetDrawColor( 255,255,255,34 * a) 
	...
	function rain.DepthFilter(w, h, a)
		...
		if rainscale > -1 then
			...
			surface.SetDrawColor( 255,255,255,154 * a * math.max(0.1, W) * WP * math.max(C,0) )
			local s,s2 = 1.7, 1.8
		...
		elseif rainscale < 1  then
			...
			surface.SetDrawColor( 255,255,255,144 * a * math.max(WP,0.1) * ((P) * 1.15) )
		...
		--	surface.SetDrawColor( 255,255,255,255 * a * WP * D  )
