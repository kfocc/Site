...
if CLIENT then
	...
	hook.Add("PreDrawOpaqueRenderables", "tfaweaponspredrawopaque", function(a, b, c)
		if c then return end
		for _, v in player.Iterator() do
			local wepv = GetActiveWeapon(v)
			if wepv != NULL then
				local dt = GetTable(wepv)
				if dt.IsTFAWeapon and dt.PreDrawOpaqueRenderables then
					dt.PreDrawOpaqueRenderables(dt)
				end
			end
		end
	end)
