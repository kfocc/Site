...
function PANEL:Setup()
	...
	---------------------------------------------------------------------------]]
	local cats = table.Copy(DarkRP.getCategories().shipments)
	...
	for _, cat in pairs(cats) do
		...
		for k, v in pairs(cat.members) do
			...
			if not buyable and not visible then bad = bad + 1 end
		end
		...
		for _, v in pairs(cat.members) do
			local buyable, visible, reason, price = hook.Call("CanBuyShipment", Eris, v)
			...
			mask.DoClick = function()
				...
				netstream.Start("DarkRP.BuyShipment", v.name)
			end
			...
			mask.DoRightClick = function()
				...
				local ask = UI_Request("Покупка - " .. v.name, "Введите желаемое количество. " .. DarkRP.formatMoney(price) .. " за 1 шт.", function(val)
					...
					UI_Derma_Query(
						...
						"Да",
						function()
							netstream.Start("DarkRP.BuyShipment", v.name, val)
						end,
						"Нет",
						function() end
					)()
...
vgui.Register("ErisF4Shipments", PANEL, "DScrollPanel")