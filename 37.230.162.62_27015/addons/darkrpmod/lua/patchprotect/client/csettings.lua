...
if sql.QueryValue("SELECT value FROM pprotect_csettings WHERE setting = 'OwnerHUD'") == "1" then sql.Query("DROP TABLE pprotect_csettings") end
-- Create SQL-CSettings-Table
if not sql.TableExists("pprotect_csettings") then sql.Query("CREATE TABLE IF NOT EXISTS pprotect_csettings ( setting TEXT, value TEXT )") end
