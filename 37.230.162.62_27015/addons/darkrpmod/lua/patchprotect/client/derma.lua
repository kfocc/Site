...
function cl_PProtect.addfrm(w, h, title, hor)
	...
	frm.title:SetFont(cl_PProtect.setFont("Inter", 25, 750, true))
...
function pan:addlbl(text, header)
	...
	lbl:SetText(text)
	lbl:SetDark(true)
	lbl:SetFont(cl_PProtect.setFont("Inter", 14, header, true))
	lbl:SizeToContents()
	self:AddItem(lbl)
	return lbl
end
...
function pan:addchk(text, tip, check, cb)
	...
	chk.Label:SetFont(cl_PProtect.setFont("Inter", 14, 500, true))
...
function pan:addbtn(text, nettext, args)
	...
	btn:SetText(text)
	btn:SetDark(true)
	btn:SetFont(cl_PProtect.setFont("Inter", 14, 500, true))
...
--------------
local sldnum = 0
function pan:addsld(min, max, text, value, t1, t2, decimals, cb)
...
function pan:addplp(ply, bud, cb, cb2)
	...
	function plp.chk:Paint(w, h)
		if not self:GetChecked() then
			...
			draw.SimpleText("r", cl_PProtect.setFont("marlett", 16, 750, false, false, true), 2, 3, color_white)
		else
			draw.RoundedBox(4, 0, 0, w, h, Color(0, 200, 75))
			draw.SimpleText("b", cl_PProtect.setFont("marlett", 22, 500, false, false, true), 0, 0, color_white)
		end
	end
...
function pan:addtxt(text)
	...
	txt:SetText(text)
	txt:SetFont(cl_PProtect.setFont("Inter", 14, 500, true))
	self:AddItem(txt)
