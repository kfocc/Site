if SERVER then resource.AddFile("materials/smartsnap_offsets_default.png") end
...
local function OnPaintHUD()
	...
	if target.locked then
		...
		if wep:GetClass() ~= 'gmod_tool' then --[[and GetConVarNumber('snap_alltools') == 0--]]
			return
		end
		...
		if class ~= 'prop_physics' and class ~= 'phys_magnet' and class ~= 'gmod_spawner' then --[[and GetConVarNumber('snap_allentities') == 0--]]
			return
		end
...
function OnPopulateToolMenu()
	spawnmenu.AddToolMenuOption("UnionRP", "Настройки", "SmartSnapSettings", "SmartSnap", "", "", OnPopulateToolPanel, {
