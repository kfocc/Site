...
local elements = {
	...
	},
	{
		...
		text = "ABORT",
		font = "KeypadAbort",
		click = function(self) self:SendCommand(self.Command_Abort) end
	},
	{
		-- OK
		x = 0.5 + 0.04 / 2 + 0.05,
		...
		text = "OK",
		font = "KeypadOK",
		click = function(self) self:SendCommand(self.Command_Accept) end
	}
}
...
do
	...
	for i = 1, 9 do
		...
		local element = {
			...
			text = tostring(i),
			click = function(self) self:SendCommand(self.Command_Enter, i) end
		}
...
function ENT:Paint(w, h, x, y)
	...
	for k, element in ipairs(elements) do
		...
		if element == hovered and element.hovercolor then surface.SetDrawColor(element.hovercolor) end
		...
		if element.render then element.render(self, cx, cy) end
	end
end
