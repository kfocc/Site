if CLIENT then
	...
	SWEP.WElements = {
		["element_name"] = {
			...
			rel = "",
			pos = Vector(-16.42, 34.831, -4.919),
			angle = Angle(-82.644, 60.391, -0.542),
			size = Vector(0.995, 0.995, 0.995),
			color = Color(255, 255, 254, 255),
...
SWEP.PrintName = "Тестовая гитара"
SWEP.Slot = 2
SWEP.SlotPos = 4
SWEP.DrawAmmo = false
...
SWEP.Secondary.Sound = Sound("none")
...
SWEP.IronSightsAng = Vector(0, 0, 0)
if CLIENT then
	netstream.Hook("UnionRP.Guitar", function(guitar, song)
		if not IsValid(guitar) then return end
		if song then
			if IsValid(guitar.station) then guitar.station:Stop() end
			sound.PlayURL(song, "3d", function(station)
				if not IsValid(station) then return end
				station:SetPos(guitar:GetPos())
				station:SetVolume(.9)
				--station:Play()
				guitar.station = station
				local nn = "update_song_" .. guitar:EntIndex()
				timer.Create(nn, 1, 0, function()
					if not IsValid(guitar) then
						station:Stop()
						timer.Remove(nn)
						return
					end

					if LocalPlayer():GetPos():Distance(guitar:GetPos()) > 1024 then
						station:Stop()
						timer.Remove(nn)
						return
					end
...
function SWEP:PrimaryAttack()
	if SERVER then
		local link, cursong = "https://unionrp.info/server_scripts/guitar/%s.mp3", math.random(1, 21)
		netstream.Start(nil, "UnionRP.Guitar", self, string.format(link, cursong))
	end
...
function SWEP:SecondaryAttack()
	if SERVER then
		local link, cursong = "https://unionrp.info/server_scripts/guitar/%s.mp3", math.random(21, 42)
		netstream.Start(nil, "UnionRP.Guitar", self, string.format(link, cursong))
	end
...
function SWEP:Reload()
	...
	if SERVER then netstream.Start(nil, "UnionRP.Guitar", self) end
end
