...
SWEP.SwingSound = {Sound("npc/fast_zombie/leap1.wav"), Sound("npc/fast_zombie/claw_miss2.wav")}
SWEP.HitSound = {Sound("npc/fast_zombie/claw_strike1.wav"), Sound("npc/fast_zombie/claw_strike2.wav"), Sound("npc/fast_zombie/claw_strike3.wav")}
SWEP.RoarSound = {Sound("npc/fast_zombie/fz_scream1.wav"), Sound("npc/fast_zombie/fz_alert_close1.wav"), Sound("npc/fast_zombie/fz_alert_far1.wav"), Sound("npc/fast_zombie/fz_frenzy1.wav")}
...
function SWEP:TranslateActivity(act)
	return anim_fast_zombie[act] or act
end
...
if SERVER then
	...
	local punchAngle = Angle(8, 0, 0)
	function SWEP:SecondaryAttack()
		self.isflying = true
		self.islanding = false
		-- self:SetVelocity((self:GetUp() * 145) + (self:GetForward() * 555))
		local owner = self:GetOwner()
		cacheTraceLine.start = owner:GetShootPos()
		cacheTraceLine.endpos = owner:GetForward() * -160
		cacheTraceLine.filter = owner
		util.TraceLine(cacheTraceLine)
		if trLineOut.Hit then
			owner:ViewPunch(punchAngle)
			owner:SetVelocity(owner:GetForward() * 555 + owner:GetUp() * 145)
			self:SendWeaponAnim(ACT_VM_SECONDARYATTACK)
			owner:DoAnimationEvent(ACT_ZOMBIE_LEAP_START)
			timer.Create(tostring(self) .. "_Animations", 0.5, 2, function()
				if not IsValid(self) then return end
				owner:DoAnimationEvent(ACT_ZOMBIE_LEAPING)
			end)

			owner:EmitSound("npc/fast_zombie/fz_scream1.wav", 60, 100, 0.7)
		end
...
if SERVER then
	hook.Add("GetFallDamage", "FastZombie", function(ply, speed)
		...
		if IsValid(plyWep) and plyWep:GetClass() == "zombieswep2" then return 0 end
	end)
else
	-- local tab = {
	-- 	["$pp_colour_addr"] = -1,
	...
	-- 	if not LocalPlayer():isZombie() then return end
	-- 	DrawColorModify(tab)
	-- end)
	function SWEP:SecondaryAttack()
	end
