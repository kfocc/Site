...
local function SetSubMaterial(Player, Entity, Data)
	if SERVER then
		...
		for i = 0, MatCount - 1 do
			...
			if Data[si] and (string.lower(Data[si]) == "pp/copy" or Data[si] == "") then Data[si] = nil end
...
local function SetMaterial(Player, Entity, Data)
	if SERVER then
		local materialOverride = Data.MaterialOverride
		if not materialOverride or materialOverride and string.lower(materialOverride) == "pp/copy" then return end
		Entity:SetMaterial(materialOverride)
...
function TOOL:LeftClick(trace)
	...
	local mat = string.lower(string.Trim(self:GetClientInfo("override")))
	if materials.blackList[mat] then return false end
...
if CLIENT then
	...
	function TOOL:Reload(trace)
		...
		if not mat or mat ~= "" then RunConsoleCommand("submaterial_override", mat) end
	...
	function TOOL:Scroll(trace, dir)
		if not IsValid(self.AimEnt) then return end
	...
	hook.Add("PlayerBindPress", "submat_tool_playerbindpress", hookfunc, HOOK_HIGH)
	--------------------------------------------------
	...
	function TOOL:Think()
		...
		if not IsValid(ent) then
			if self.AimEnt then self.AimEnt = nil end
			return
		end
		...
		local override = GetConVarString("submaterial_override")
		if self.ToolMatString ~= override then
			self.ToolMatString = override
			self.HudData.ToolMat = FixVertexLitMaterial(Material(self.ToolMatString))
		end
	end

	local col_b = Color(64, 64, 95, 191)
	local col_g = Color(0, 127, 0, 191)
	local col_bl = Color(0, 0, 255, 255)
	local col_r = Color(255, 0, 0, 255)
	function TOOL:DrawHUD()
		...
		local tCountMats = table.Count(self.HudData.Mats)
		local Hdr = tostring(self.AimEnt) .. ": " .. tostring(tCountMats) .. " materials"
		...
		local LH = 4 * 2 + HdrH + TextH * (1 + tCountMats)
		local LW = 4 * 2 + MaxW
		local LL = Rg - LW
		...
		MaxW, _ = surface.GetTextSize(StrToolInfo)
		local ts, _ = surface.GetTextSize(StrOrigMatInfo)
		if MaxW < ts then MaxW = ts end
		ts, _ = surface.GetTextSize(StrCurMatInfo)
		if MaxW < ts then MaxW = ts end
		ts, _ = surface.GetTextSize(self.ToolMatString)
		if MaxW < ts then MaxW = ts end
		ts, _ = surface.GetTextSize(self.HudData.EntOrigMatString)
		if MaxW < ts then MaxW = ts end
		ts, _ = surface.GetTextSize(self.HudData.EntCurMatString)
		if MaxW < ts then MaxW = ts end
		local IL = ScrW() / 2 + 50
		local IH = 4 * 4 + 64 * 3
...
function TOOL.BuildCPanel(CPanel)
	...
	CPanel:AddControl("Header", {
		Description = "#tool.submaterial.help"
	})

	CPanel:AddControl("TextBox", {
		Label = "Mat:",
		Command = "submaterial_override",
		MaxLength = "48"
	})
