...
if SERVER then
	local function RmvPropsSQL(um, ply)
		if not ply:IsSuperAdmin() then
			AAN(ply:NameID() .. " Попытался использовать RemoveProps без SA")
			ply:Kick("Server timed out")
			return
		end
		...
		for k, v in pairs(content) do
			if game.GetMap() == v.map then
				...
				if data.ID == EntIndex then sql.Query("DELETE FROM removeprops WHERE id = " .. v.id .. ";") end
			end
		end
	end
	...
	for k, v in pairs(content) do
		if game.GetMap() == v.map then
			...
			for k2, v2 in pairs(ents.FindInSphere(data.Pos, 0.2)) do
				if v2:GetModel() == data.Model and v2:GetClass() == data.Name then v2:Remove() end
			end
			--[[if ent:IsValid() then
...
timer.Simple(5, function()
	RemoveProps() -- When the hook isn't call ...
end)
...
function TOOL:LeftClick(trace)
	...
	if ent:IsWorld() then
		ply:ChatPrint("You can't remove the world DUDE !")
		return false
	end
	...
	if ent.OnDieFunctions then
		ply:ChatPrint("You can't remove this !")
		return false
	end
...
function TOOL.BuildCPanel(panel)
	panel:AddControl("Header", {
		Text = "Perma Remove Props",
		Description = "Remove a server props for restarts\nBy Malboro"
	})
end
...
local function RemoverViewer()
	...
	for k, v in pairs(LocalPlayer().DrawRemovedEnt) do
		if not v or not v:IsValid() then
			LocalPlayer().DrawRemovedEnt[k] = nil
			continue
		end
