...
if SERVER then CreateConVar("sbox_maxledscreens", 5) end
...
if CLIENT then
	language.Add("Tool.ledscreen.name", "LED панели")
	language.Add("Tool.ledscreen.desc", "Создать LED панель")
	language.Add("Tool.ledscreen.left", "Поставить панель")
	language.Add("Tool.ledscreen.right", "Обновить панель")
...
function TOOL:Reload(tr)
	local TraceEnt = tr.Entity
	if not IsValid(TraceEnt) then return false end
	if TraceEnt:IsValid() and TraceEnt:GetClass() == "gb_rp_sign" then
		if CLIENT then
...
function TOOL.BuildCPanel(CPanel)
	...
	frame.Paint = function(self, w, h)
		...
		self.static = false
		local ww = surface.GetTextSize(self.Text)
		local now = SysTime()
		...
		if self.Type == 1 then
			...
			draw.SimpleText(self.Text, font, xs, 0, self.Color, TEXT_ALIGN_LEFT)
		elseif self.Type == 2 then
			if not self.appr or self.appr > ww then
				self.appr = -w
			else
				self.appr = math.Approach(self.appr, ww + w, approachMult)
			end

			draw.SimpleText(self.Text, font, self.appr * -1, 0, self.Color, TEXT_ALIGN_LEFT)
		else
			if not self.appr then
				self.appr = 1
				self.refl = true
			end
			...
			if w > ww then
				if self.Type == 3 then
					if self.appr < w - ww and not self.refl then
						self.appr = math.Approach(self.appr, ww + w, approachMult)
					else
						if self.appr <= 0 then
							self.refl = nil
							...
							self.refl = true
							self.appr = math.Approach(self.appr, 0, approachMult)
						end
					end
				else
					self.static = true
				...
				if self.appr > w - ww - 50 and not self.refl then
					self.appr = math.Approach(self.appr, w - ww - 50, approachMult)
				else
					if self.appr >= 50 then
						self.refl = nil
					else
						self.refl = true
						self.appr = math.Approach(self.appr, 50, approachMult)
					end
				end
			end

			if self.static then
				draw.SimpleText(self.Text, font, w / 2, 0, self.Color, TEXT_ALIGN_CENTER)
			else
				draw.SimpleText(self.Text, font, self.appr, 0, self.Color, TEXT_ALIGN_LEFT)
			end
		end
	...
	CPanel:AddControl("CheckBox", {
		Label = "Эффект мерцания",
		Description = "",
		Command = "ledscreen_fx"
	})
