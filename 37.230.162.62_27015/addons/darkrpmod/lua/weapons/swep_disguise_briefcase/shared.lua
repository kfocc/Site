...
function SWEP:Deploy()
	...
	self:GetOwner().Deceive_Using = self
	return true
end

function SWEP:Holster()
	self:GetOwner().Deceive_Using = nil
	self.User = nil
	return true
end
...
function SWEP:Reload()
	if (self.nextr or 0) > CurTime() then return end
	self.nextr = CurTime() + 1
	if not self:GetOwner().Disguised then return end
	if SERVER then self:GetOwner():Disguise(nil) end
end
