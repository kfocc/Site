...
function SWEP:Initialize()
	...
	timer.Create("medkit_ammo" .. self:EntIndex(), 1, 0, function() if self:Clip1() < self.MaxAmmo then self:SetClip1(math.min(self:Clip1() + 2, self.MaxAmmo)) end end)
end
	...
	if IsValid(ent) and self:Clip1() >= need and (ent:IsPlayer() or ent:IsNPC()) and ent:Health() < ent:GetMaxHealth() then
		...
		if owner:Team() == TEAM_GSR4 or owner:Team() == TEAM_GANG4 then
			...
			if n_heal and n_heal < CurTime() or self_now < 1000 then
				owner:addMoney(need * 5, "Лечение жителей")
				self.now = self_now
			end
		end
	else
