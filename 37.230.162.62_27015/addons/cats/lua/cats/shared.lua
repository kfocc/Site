...
cats.lang = {
    ...
    playTimeCard = "Наигранное время за последнюю неделю",
    ticketClaimCard = "Принятые жалобы за последнюю неделю",
    ok = "Готово",
    cancel = "Отмена",
...
cats.config.serverID = "city20"
cats.config.getPlayerName = function(ply)
    if not IsValid(ply) then return "unknown(not valid)" end
    return ply:Name() .. " (" .. ply:SteamName() .. ")"
end

cats.config.allowedUsergroups = {
    superadmin = true,
...
function cats.config:playerCanSeeTicket(ply, ticketSteamID)
    return self.allowedUsergroups[ply:GetUserGroup()] and ply:Team() == TEAM_ADMIN or ply:SteamID() == ticketSteamID
end

function cats:isSteamID(sid)
    sid = isstring(sid) and sid:IsSteamID32()
	if sid then
		return sid
	else
		return false
...
cats.config.commands = {
    ...
    { -- bring
        ...
        click = function(ply)
            ...
            netstream.Start("cats.admin_actions", "ulx_bring", ply)
        end
    },
    { -- return
        ...
        click = function(ply)
            ...
            netstream.Start("cats.admin_actions", "ulx_return", ply)
        end
    },
    { -- goto
        ...
        click = function(ply)
            ...
            netstream.Start("cats.admin_actions", "ulx_goto", ply)
        end
    },
    { -- return self
        ...
        click = function(ply)
            ...
            netstream.Start("cats.admin_actions", "ulx_return", LocalPlayer())
        end
    },
    {
        text = cats.lang.action_tobase,
        icon = 'house_go',
        click = function(ply)
            -- netstream.Start("cats.to_admin_base", ply)
            netstream.Start("cats.admin_actions", "ulx_base", ply)
        end
    },
    { -- copy steamID
