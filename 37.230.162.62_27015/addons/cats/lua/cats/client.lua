-- luacheck: globals netstream CreateClientConVar surface cats ErrorNoHalt db mysqloo util surface draw Material net Color timer vgui
        ...
        click = function(e, a)
            ...
            for n, t in ipairs(cats.config.commands) do
                e:AddOption(t.text, function()
                    t.click(a)
                    netstream.Start("cats.action_used", a:SteamID())
...
local function i(c)
    ...
    e.Paint = function(n, t, a)
        ...
        local a = "(" .. os.date("%M:%S", CurTime() - (n.ticket.created or 0)) .. ")"
        local n = IsValid(c) and c:Name() or cats.lang.userDisconnected
        local tName = IsValid(c) and team.GetName(c:Team()) or "Unknown"
        -- local c = "★" .. math.Round(c:GetNetVar("cats_averageRating", 0), 1)
        -- draw.SimpleText(a .. " " .. c .. " " .. n, "cats.small", 8, 15, Color(220, 220, 220), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText(a .. " " .. n .. "( " .. tName .. " )", "cats.small", 8, 15, Color(220, 220, 220), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

        if IsValid(o) then
            -- local e = "★" .. math.Round(o:GetNetVar("cats_adminRating", 0), 1)
            -- draw.SimpleText(e .. " " .. o:Name(), "cats.small", t - 8, 15, Color(180, 200, 240), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
            draw.SimpleText(o:Name(), "cats.small", t - 8, 15, Color(180, 200, 240), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
        end
    end
    ...
    t.Paint = function(e, n, a)
        ...
        draw.SimpleText(cats.lang.sendMessage, "cats.small", 8, 10, Color(220, 220, 220, 220), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
...
hook.Add("Think", "cats", function()
    ...
    e:SetRenderInScreenshots(false)
    cats.ticketFrame = e
...
local function d(e)
    ...
    e.Paint = function(e, a, n)
        ...
        surface.SetDrawColor(col.o)
        surface.DrawRect(0, 0, a, 30)
        surface.SetDrawColor(0, 0, 0, 255)
        surface.DrawOutlinedRect(0, 0, a, n)
        local e = "(" .. os.date("%M:%S", CurTime() - e.ticket.created) .. ")"
        if IsValid(t) then
            -- e = e .. ": ★" .. math.Round(t:GetNetVar("cats_adminRating", 0), 1) .. " " .. t:Name()
            e = e .. ": " .. t:Name()
            -- draw.SimpleText(e .. " " .. t:Name(), "cats.small", a - 8, 15, Color(180, 200, 240), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
        else
            e = e .. " " .. cats.lang.myTicket
        end
        draw.SimpleText(e, "cats.small", 8, 15, Color(220, 220, 220), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
...
netstream.Hook("cats.closeTicket", function(e, b)
    if e == LocalPlayer():SteamID() and t then
        if IsValid(t.admin) and not b then
...
netstream.Hook("cats.syncTickets", function(e)
...
--end)
--
--concommand.Add("cats_test_myticket_clear", function()
--    if IsValid(cats.myTicketFrame) then
--        cats.myTicketFrame:Remove()
--    end
--end)
...
netstream.Hook("cats.openAnalytics", a)
...
netstream.Hook("cats.getAdminList", function(e)
    ...
    for a, e in ipairs(e) do
        ...
        local l = cats.analyticsFrame.list:AddLine(t, e.lastNick)
        l.steamID = e.steamID
    end
...
local usergroups = {
    ...
    vip = "VIP",
    operator = "Оператор",
    operator_nabor = "Наборный оператор",
    moderator = "Модератор",
    moderator_nabor = "Наборный модератор",
    admin = "Администратор",
    admin_setting = "Администратор",
    administrator = "Администратор",
    administrator_nabor = "Наборный администратор",
    head_admin_nabor = "Смотритель",
    advisor_nabor = "Советник",
    event_boss_nabor = "Главный ивентолог",
    event_nabor = "Ивентолог",
    overwatch = "Overwatch",
    superadmin = "Основатель"
}
-- local function weeks(num, e)
--     return (num or 0) / (e.updateTime - e.createdTime) * 604800
-- end
-- local function months(num, e)
--     return (num or 0) / (e.updateTime - e.createdTime) * 2592000
-- end
-- --net.Receive("cats.getAdminData", function()
netstream.Hook("cats.getAdminData", function(e)
    --local e = net.ReadTable()
    ...
    t:SetText(usergroups[e.userGroup] or e.userGroup)
    t:SetContentAlignment(5)
    ...
    local n = {
        {"РЕЙТИНГ", "★" .. math.Round(e.ratingTotal or 0, 2)},
        {"В МЕСЯЦ", '★' .. math.Round(e.ratingMonth or 0, 2)},
        {'ПОЛЬЗОВАТЕЛИ', e.uniqueUsers},
        {'ЖАЛОБЫ', e.claimsTotal},
        {'В МЕСЯЦ', math.Round(e.claimsMonth or 0)},
        {'ВРЕМЯ ЖАЛОБ', r(e.ticketTimeTotal)},
        {'БАНОВ', math.Round(e.bans)},
        {'ДЖАЙЛОВ', math.Round(e.jails)},
        {'ВАРНОВ', math.Round(e.warns)},
        {},
    }
    ...
    for e, n in ipairs(n) do
        ...
        e.Paint = function(o, t, a)
            ...
            draw.RoundedBox(8, 5, 5, t - 10, a - 10, Color(0, 0, 0, 100))
    ...
    t:SetTall(20)
    t:SetFont('cats.medium')
    t:SetText(cats.lang.playTimeCard)
    ...
    n.Paint = function(t, e, a)
        draw.RoundedBox(8, 0, 0, e, a, Color(0, 0, 0, 100))
        e, a = e - 55, a - 35
        local n, e = a / 7, e / 24
        draw.NoTexture()

        for e = 1, 7 do
            draw.SimpleText(d[e], "cats.medium", 25, 30 + n * (e) - n / 2, Color(220, 220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        ...
        for o = 1, 24 do
            local t = o - r
            t = t > 0 and t or t + 24
            draw.SimpleText(o ~= 24 and o or 0, "cats.small", 50 + e * (t - 1) + e / 2, 20, Color(220, 220, 220, 220), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            surface.SetDrawColor(220, 220, 220, 4)
    ...
    t:SetTall(20)
    t:SetFont('cats.medium')
    t:SetText(cats.lang.ticketClaimCard)
    local l = 1
...
netstream.Hook("cats.refreshTickets", function()
    if IsValid(cats.ticketContainer) then
        cats.ticketContainer:Clear()
    end
end)