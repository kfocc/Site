--[[
	...
	Кодирование в hex дает преимущество:
	- В некоторых случаях заметно увеличивает скорость сериализации.
	- Уменьшает размер строки на массивах с числовыми данными.

	По умолчанию смешанные таблицы записываются начиная с последовательностей, потом как пара ключ-значение.
	В данной реализации если в таблице есть ключ 0 то next вернет 0, в результате чего вся таблица будет записана
	как пара ключ-значение.
--]]
-- luacheck: globals pon

cats.pon = cats.pon or {}
...
do
	...
	encode["table"] = function(tbl, output, cache)
		if cache[tbl] then
			insert(output, format("(%x)", cache[tbl]))
			return
		else
