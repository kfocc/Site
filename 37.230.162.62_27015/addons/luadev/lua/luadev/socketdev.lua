...
local function requireExists(moduleName)
	local osSuffix = assert((system.IsWindows() and (jit.arch ~= "x64" and "win32" or "win64")) or (system.IsLinux() and "linux") or (system.IsOSX() and "osx"), "couldn't determine system type?")
...
for _, moduleName in ipairs({"socket", "luasocket"}) do
	if requireExists(moduleName) then
		...
		if not ok then
			luadevPrint(string.format("Unable to load module %s: %s", moduleName, err))
		else
			if not socket then
