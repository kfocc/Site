...
local function addLaw(law)
	...
	hook.Run("addLaw", lawNumber, law)
end

netstream.Hook("DRP_ChangeLaw", function(id, law)
	Laws[id] = law
	hook.Run("changeLaw", id, law)
end)
...
netstream.Hook("DRP_GetLaws", function(laws)
	Laws = table.Copy(laws)
end)

netstream.Hook("DRP_LoadLaws", function(laws)
	Laws = table.Copy(laws)
	hook.Run("loadLaws")
end)
