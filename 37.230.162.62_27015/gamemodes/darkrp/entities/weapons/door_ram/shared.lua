...
SWEP.Author = "UnionRP"
SWEP.Instructions = "ПКМ: Подготовить таран\nЛКМ: Выбить дверь или проп"
SWEP.Contact = ""
SWEP.Purpose = ""
...
local function ramFadingDoor(ply, trace, ent)
    ...
    if  ent:GetNetVar("fadingfix") then
        ent:Fade()
    end]]
    if ply:EyePos():DistToSqr(trace.HitPos) > 10000 then return false end
    ...
    timer.Create(ent:GetCreationID() .. "toremove", 5, 1, function()
        if IsValid(ent) and ent.toremove then
            ent:Remove()
        end
    end)
...
local function ramProp(ply, trace, ent)
    ...
    local isTextScreen = acceptedProps[entClass]
    if entClass ~= "prop_physics" and not isTextScreen then return false end
    if isTextScreen then
        local hasCrm = false
        for k, v in player.Iterator() do
            if v:Team() == TEAM_CREMATOR then
                hasCrm = true
                break
            end
        end
        if hasCrm then
            if SERVER then
                DarkRP.notify(ply, 1, 5, "Поручите заняться этим крематору.")
            end
            return false
        end
    end
    ...
    ent.toremove = true
    timer.Create(ent:GetCreationID().."toremove",5,1,function() if IsValid(ent) and ent.toremove then ent:Remove() end end)
