...
local function addEntityCommands(tblEnt)
    ...
    local function buythis(ply, args)
        if ply:isArrested() and tblEnt.ent ~= "arrest_spawn_basket" then return "" end
        ...
        DarkRP.CollectEntities[uid]:Push(ent)
...
-- RPExtraTeams = {}
if not istable(RPExtraTeams) then
    table.Empty(RPExtraTeams)
else
    RPExtraTeams = {}
end
...
plyMeta.getJobTable = function(ply)
    local tbl = RPExtraTeams[ply:Team()] or {}
    return tbl
end

function DarkRP.getJobTableByIndex(indexTeam)
    local tbl = RPExtraTeams[indexTeam] or {}
    return tbl
end
...
function DarkRP.createJob(Name, colorOrTable, model, Description, Weapons, command, maximum_amount_of_this_class, Salary, admin, Vote, Haslicense, NeedToChangeFrom, CustomCheck)
    ...
    if istable(CustomTeam.model) then
        for k, v in pairs(CustomTeam.model) do
            v = string.lower(v)
            CustomTeam.model[k] = v
            util.PrecacheModel(v)
        end
    else
        CustomTeam.model = string.lower(CustomTeam.model)
