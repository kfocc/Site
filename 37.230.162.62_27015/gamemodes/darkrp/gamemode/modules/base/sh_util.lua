...
function DarkRP.findPlayer(info)
    ...
    for k = 1, #pls do
        ...
        if (info == v:SteamID()) then
            return v
        elseif (tonumber(info) == v:SteamID64()) then
            return v
        elseif (tonumber(info) == v:UserID()) then
            return v
        elseif string.find(string.lower(v:Name()), string.lower(info), 1, true) ~= nil then
            return v
        elseif string.find(string.lower(v:SteamName()), string.lower(tostring(info)), 1, true) ~= nil then
            return v
        end
    end
