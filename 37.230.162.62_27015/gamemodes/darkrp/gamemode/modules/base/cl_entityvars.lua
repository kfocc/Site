local tagName = "DRPVar."
--[[---------------------------------------------------------------------------
interface"someString"
---------------------------------------------------------------------------]]

local eMeta = FindMetaTable("Entity")
local getNetVar = eMeta.GetNetVar

local tags = {}
_G.tags = tags

local function checkTag(var)
	local tag = tags[var]
	if not tag then
		tags[var] = tagName .. var
		tag = tags[var]
	end
	return tag
end
...
function pmeta:getDarkRPVar(var, default)
	local tag = checkTag(var)
	return getNetVar(self, tag, default)
end
